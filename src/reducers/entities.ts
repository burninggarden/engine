...
function handleAddEntityAction(state: Entity[], action: AddEntityAction): Entity[] {
	return state.concat(action.entity);
}

function handleSetEntityPositionAction(state: Entity[], action: SetEntityPositionAction): Entity[] {
	return updateItemInList(state, action.entity_id, (entity) => {
...
function EntitiesReducer(state: Entity[] | undefined, action: BaseAction) : Entity[] {
	if (state === undefined) {
		return [];
	}
	...
	switch (action.type) {
		case ActionType.ADD_ENTITY:
			return handleAddEntityAction(
				state,
				action as AddEntityAction
			);
